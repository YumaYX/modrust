...
impl fmt::Display for InstructionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "invalid instruction number: {}", match self {
            InstructionError::InvalidNumber(n) => n,
        })
    }
}
...
fn instruction(number: u8) -> Result<String, InstructionError> {
    match number {
        ...
        3 => Ok("Please add or update rustdoc(cargo doc) comments for the following Rust code.".to_string()),
        n => Err(InstructionError::InvalidNumber(n)),
    }
}

fn build_prompt(number: u8, rust_code: &str) -> Result<String, Box<dyn Error>> {
